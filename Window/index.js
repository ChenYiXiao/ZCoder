...
windows["MenuTree"] = { object: null, type: 0, title: "风声 JS 菜单树", features: "height=-1", x: 335, y: parseInt(Math.random() * 300) };
windows["FlcSelect"] = { object: null, type: 0, title: "风声无限级联下拉列表/菜单", features: "height=-1", x: 335, y: parseInt(Math.random() * 300) };
windows["FbaFrameset"] = { object: null, type: 0, title: "风声后台管理框架集", features: "height=-1", x: 335, y: parseInt(Math.random() * 300) };
windows["Others"] = { object: null, type: 0, title: "风声日期时间选择器及其它", features: "height=-1", x: 335, y: parseInt(Math.random() * 300) };
        ...
        case 0:
            ...
            switch (id) {
                case "Option":
                    ...
                    break;
                case "Welcome":
                    win.OnOK = function () { win.Close(); OpenWindow("Works"); };
...
function NewWindow() {
    ...
    dialog.OnHELP = function () {
        _window.Alert("[id]AboutWindow", "关于风声 JS 窗口", "isModal=yes,width=360,height=180,right=100,top=100");
    }
...
function Init() {
    _window.ClassName = getCookie("FonshenStyle") || "NOKIA";
